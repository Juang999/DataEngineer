...
from mutif_etl.transform.dim.dim_currency import transform_data_curerncy
...
if __name__ == '__main__':
    ...
    file_marker_query = Path(__file__).parent.parent.parent.joinpath(r"queries/inc/marker/dim/dim_currency.sql")
    file_dim_query = Path(__file__).parent.parent.parent.joinpath(r"queries/inc/dim/dim_currency.sql")
    ...
    transformed_data = transform_data_curerncy(extracted_data_exapro)

    load_dataframe(connection=env_data_warehouse, schema="public", tablename="dim_currency", mode="append", df=extracted_data_exapro)
