...
from mutif_etl.transform.fact_sales_shipment import transform_data_shipment
from mutif_etl.load.load_dataframe import load_dataframe

if __name__ == "__main__":
    config = Environment()

    env_src_db_exapro = config.env_source_exapro
    env_dst_db_warehouse = config.env_destination_warehouse
    file_query = Path(__file__).parent.parent.parent.joinpath("queries\init_fact_sales_shipment.sql")
